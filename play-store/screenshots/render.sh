...
USER_DATA="$(mktemp -d /tmp/chrome-play-ss-XXXXXX)"
PORT=9500
for html in html/*.html; do
  ...
  # Play Console: JPEG under 8MB, 9:16, sides 320–3840. 2x render = 2160x3840.
...
print("--- JPG sizes ---")
for p in sorted(Path("out").glob("*.jpg")):
    print(f"{p.name:24} {p.stat().st_size/1024:.0f}KB")
PY
ls -lh out
