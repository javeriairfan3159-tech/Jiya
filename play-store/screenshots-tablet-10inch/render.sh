...
USER_DATA="$(mktemp -d /tmp/chrome-play-tab10-XXXXXX)"
PORT=9700
for html in html/*.html; do
  ...
  PORT=$((PORT + 1))
  echo "Rendering $base"
  timeout 40s "$CHROME" --headless=new --disable-gpu --hide-scrollbars --no-sandbox \
    --user-data-dir="$USER_DATA" \
    --remote-debugging-port="$PORT" \
    --disable-background-networking --disable-sync \
    --force-device-scale-factor=2 --window-size=2560,1440 \
